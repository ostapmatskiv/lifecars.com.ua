				...
				success:function (res) {
					if(res['result'] == true){
						...
						$.each(res['user'], function (index,value) {
							$("#userTable>table>tbody").append("<tr><td>"+value.id+"</td><td>"+value.name+"</td><td><a href=\"<?=SITE_URL?>admin/wl_users/"+value.email+"\" target='_blank'>"+value.email+'</a><br>'+value.user_phone+"</td><td><button onclick='setUser("+value.id+", "+value.type+")' class='btn btn-xs btn-success'><i class='fa fa-check'></i></button></td></tr>");
						})
					} else {
						$("#userTable").hide();
