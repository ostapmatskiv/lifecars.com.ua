...
class cart_model
{
	...
	public function getProductsInCart($user = 0, $active = 1)
	{
		$where_cp = array('cart' => 0);
		if($active)
			$where_cp['active'] = 1;
		$where_cp['user'] = ($user == 0) ? $this->getUser() : $user;
	...
	{
		if(!empty($_SESSION['user']->id) && !empty($_SESSION['cart']->user) && $_SESSION['cart']->user < 0)
			$this->db->updateRow($this->table('_users'), ['user' => $_SESSION['user']->id], -$_SESSION['cart']->user);
		...
		if(!empty($_COOKIE['cart_id']))
		{
			if($user = $this->db->getAllDataById($this->table('_users'), trim($_COOKIE['cart_id']), 'cookie'))
			{
				...
				{
					$this->db->updateRow($this->table('_users'), ['user' => $_SESSION['user']->id], $user->id);
					$this->db->updateRow($this->table('_products'), array('user' => $_SESSION['user']->id), array('cart' => 0, 'user' => -$user->id));
	...
	public function updateAdditionalUserFields($user_id)
	{
       ...
       //          		}
                	...
                	else
                	{
                		$data = array('user' => $user_id, 'date' => time());
						$data['field'] = $key;
						$data['value'] = $_SESSION['user']->$key = $value;
	        			$this->db->insertRow('wl_user_info', $data);
                	}
                }
            }
