...
class cart_admin extends Controller {
    ...
    private function sendMail_Change_status($cartId, $comment = '')
    {
        ...
        if($cart = $this->cart_model->getById($cartId))
        {
            ...
            $info['payment'] = false;
            if($cart->payment_alias == 0 && $cart->payment_id)
    ...
    public function findUser()
    {
        ...
        if($this->data->post('userInfo'))
        {
            ...
            $users = $this->db->getQuery("SELECT u.id, u.email, u.name, t.title as type_name, ui.value as user_phone FROM `wl_users` as u 
