	...
	public function index ($uri) {
		...
		if (is_numeric($uri)) {
			if ($payment = $this->liqpay_model->getPayment($uri)) {
				$_SESSION['alias']->breadcrumb = [
					$_SESSION['alias']->name => 'admin/' . $_SESSION['alias']->alias,
					'Оплата #' . $uri        => ''
				];
			...
			$this->load->admin_view('list_view', ['payments' => $this->liqpay_model->getPayments()]);
		}
	}
	...
	public function test () {
		...
		if ($pay_id = $this->data->post('pay_id')) {
			if ($pay = $this->liqpay_model->getPayment($pay_id)) {
				...
				$curl_data = ['data' => $data, 'signature' => $signature];
