...
class shopshowcase_admin extends Controller {
	...
	{
		if($similar = $this->db->getAllDataById('s_shopshowcase_products_similar', $this->data->post('id'), 'product'))
		{
			$this->__after_edit($similar->id);
			$this->db->deleteRow('s_shopshowcase_products_similar', $similar->id);
			if($this->db->getCount('s_shopshowcase_products_similar', $similar->group, 'group') == 1)
